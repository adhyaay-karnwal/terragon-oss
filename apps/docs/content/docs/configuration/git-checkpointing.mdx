...
- **Automatic commits**: When a task finishes, Rover automatically commits all changes made during the session to the remote branch with descriptive commit messages and PR descriptions
...
You may want to disable git checkpointing if you're working alone and want to commit directly to the default branch, or use 3rd party tools that may interfere with Rover's git checkpointing (eg like Graphite), or otherwise have custom git workflows.

Git checkpointing can be controlled on a per-task basis when creating a new task:

<Steps>
  <Step>
    Navigate to the [**Home**](https://www.roverlabs.com) page
  </Step>
