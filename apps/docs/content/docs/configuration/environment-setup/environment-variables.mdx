...
description: Configure secrets and environment variables for your Rover sandboxes.
---
...
Rover automatically sets the following environment variables in your sandbox:

| Name       | Description                                                                                                                                                              |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `ROVER` | Always set to `true` when running in a Rover sandbox. You can use this variable to differentiate when the agent/code is running in a Rover sandbox versus locally. |
| `TERRAGON` | Back-compat alias for `ROVER`. |
| `GH_TOKEN` | GitHub access token for the `gh` CLI. You can override this by setting your own `GH_TOKEN` environment variable.                                                         |

<Callout type="info">
  **Custom GitHub Token**: If you need to use a different GitHub token (e.g., for accessing private packages or repositories), you can add your own `GH_TOKEN` environment variable. Your custom token will take precedence over the default token provided by Rover.
</Callout>
...
Environment variables are automatically available in your `rover-setup.sh` script:

```bash title="rover-setup.sh"
#!/bin/bash
