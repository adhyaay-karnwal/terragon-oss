---
title: Rover CLI & MCP
description: Command-line interface for interacting with Rover that can also be used as an MCP server
---
...
<img
  src="https://img.shields.io/npm/v/@rover-labs/cli.svg?style=flat-square"
  alt="npm"
/>

The Rover CLI let's you interact with Rover directly from the command line. Create tasks, review changes, pull tasks to pick up where you left off, all from your terminal.
...
<Callout type="warning">
  **Note**: Currently only npm is supported for installing the Rover CLI.
</Callout>
...
<Steps>
  ...
  <Step>Store credentials safely in `~/.rover/config.json`</Step>
  <Step>Confirm successful connection</Step>
</Steps>

You can override the default settings directory by setting the `ROVER_SETTINGS_DIR` environment variable before running the CLI. For example:
...
rover create "Refactor code" --model opus

# Create a task in plan mode (for complex tasks requiring planning)
rover create "Build new authentication system" --mode plan
```
...
<Callout type="info">
  **Getting the task ID**:
  ...
  Rover. For example, in `https://roverlabs.com/tasks/abc123-def456`, the
