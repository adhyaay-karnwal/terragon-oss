---
title: Release Notes
description: Stay up to date with the latest changes and improvements to Rover.
---
...
- **Dynamic model selection with GitHub mentions**: When mentioning @rover-labs in GitHub issues or PRs, you can now specify which model to use for the task by including it in brackets, e.g., `@rover-labs [model-name] your request here`. [Learn more](/docs/integrations/github-integration#model-selection).
...
- **GPT-5.1**: GPT-5.1 and GPT-5.1 Codex models are now supported.
  - You can still access the older GPT-5 models by enabling them in [Settings](https://www.roverlabs.com/settings/agent).
- **Gemini 2.5 Pro**: The OpenCode agent now supports Gemini 2.5 Pro. Enable this model in [Settings](https://www.roverlabs.com/settings/agent) to use it.
...
![Multiple credentials](https://cdn.roverlabs.com/multiple-credentials-1zLJ.png)
...
![Branch control](https://cdn.roverlabs.com/branch-toggle-vTKc.png)
...
![Task forking](https://cdn.roverlabs.com/fork-task-yW3l.png)
...
- **Configure available agents and models**: You can now configure which available agents and models show up in the model selector in prompt boxes.
  ...
  ![Agent settings](https://cdn.roverlabs.com/agent_settings-cCI5.png)

- **New features in [early access](/docs/resources/early-access-features)**:

  - **Git diff viewer and file tree**: View diffs in a responsive secondary panel inside of tasks.

    ![Diff viewer](https://cdn.roverlabs.com/diff_viewer-gH9X.png)
...
- **GitHub Mention Automation**: Customize the behavior when @rover-labs is mentioned in GitHub by creating a "GitHub Mention" automation.
...
- **Codex Rate Limit Queueing**: When you hit a rate limit associated with your ChatGPT subscription, Rover automatically queues new tasks and messages until your rate limit resets. This behavior is similar to how we handle Claude rate limit errors and should help make it easier to continue working when you hit a rate limit.

![Multi-Hour Schedule Automations](https://cdn.roverlabs.com/multi-hour-schedule-8b3S.png)
...
- **Haiku 4.5**: Rover now supports Haiku 4.5 when using the Claude Code agent. Learn more about this new model [here](https://www.anthropic.com/news/claude-haiku-4-5).
...
- **New features in [early access](/docs/resources/early-access-features)**:
  - **Multi-Agent Mode**: Run multiple AI agents from one prompt. This is useful when you want to compare different approaches or if you're not sure if a specific model is the best fit for your task.
    ![Multi-Agent Mode](https://cdn.roverlabs.com/multi-agent-mode-5K1D.png)
...
- **Slack Integration**: Create tasks directly in Slack! Connect your Slack workspace from the new [Integration Settings page](https://www.roverlabs.com/settings/integrations), where you can also set a default repo and default model for tasks created from Slack. Check out the [Slack integration documentation](/docs/integrations/slack-integration) for setup instructions and usage examples.

  ![Slack integration settings](https://cdn.roverlabs.com/slack-integration-QVNL.png)
...
- **Environment Management**: You can now delete environments and their associated data after selecting an environment from the [Environments](https://www.roverlabs.com/environments) page.
...
- **Documentation Updates**:
  ...
  - New ["When to use Rover"](/docs/getting-started/quick-start#when-and-how-to-use-rover) section to help understand ideal use cases
...
- **Amp Agent**: Rover now supports [Amp](https://ampcode.com). Configure your Amp API key in [Settings](https://www.roverlabs.com/settings/agent) to use this powerful alternative to Claude and Codex.
- **Sonnet 4.5**: Rover now supports Sonnet 4.5 when using the Claude Code agent.
- **GitHub Integration Improvements**:
  - When you mention `@rover-labs` in a PR comment, we now include more context from the full comment thread
  ...
  - You can now specify a default model for new tasks when @rover-labs is mentioned. Update this in [Settings](https://www.roverlabs.com/settings/github)
...
- Running the `rover pull` CLI command now checks for uncommitted changes before switching branches
...
- **Schedule individual tasks**: You can now schedule individual tasks to run at a specific time in the future from the prompt box. Note: If you want to run a task on a recurring schedule, do so from the [Automations](https://www.roverlabs.com/automations) page.
  ![Schedule tasks](https://cdn.roverlabs.com/schedule-tasks-esMN.png)

- **GitHub issue and pull request automation triggers**: Automations can now be triggered when GitHub issues and pull requests are created or updated in a repository. Helpful for automatically triaging new GitHub issues, creating tasks to implement a GitHub issue, or doing custom code reviews on pull requests. Accessible from the [Automations](https://www.roverlabs.com/automations) page. Check out our docs for [example prompts](/docs/tasks//automations#example-use-cases).
...
- **Branch name prefix customization**: You can now customize the branch prefix added to Rover-generated branches (defaults to `rover/`). Update this in [Settings](https://www.roverlabs.com/settings).
- **Codex Agent Improvements**: MCP tool calls and errors are now rendered and surfaced in the task output
- **Support for Claude Teams and Enterprise**: You can now use Claude Teams and Claude Enterprise subscriptions in Rover
- **Rover Sandbox Environment Variable**: Added `ROVER` environment variable (with `TERRAGON` as a back-compat alias) to sandbox environments so you can distinguish when the agent is running in a Rover sandbox versus elsewhere.
- **Setup Script Editor Improvements**: The setup script editor now streams output in real-time so you'll see live feedback as you test changes. Access this by clicking on one of your environments from the [Environments](https://www.roverlabs.com/environments) page.
...
- Fixed page load performance when loading roverlabs.com. There were some cases of very slow connections, which are now resolved.
...
- **Edit GitHub action workflows**: Rover can now create and edit GitHub Action workflows in your repository
- **CLI Enhancements**:
...
- **Connect your ChatGPT subscription to use with Codex**: You can now use an existing ChatGPT subscription to power the Codex agent instead of providing an OpenAI API key. Just go to [Settings](https://roverlabs.com/settings) and navigate to the ChatGPT Account section. If you already have an OpenAI API key connected, you'll need to disconnect that first if you want to connect your ChatGPT subscription.
...
![New features](https://cdn.roverlabs.com/new-EcZ2.png)
...
- Fixed a bug where Codex didn't respect custom system prompts, available in [Settings](https://roverlabs.com/settings)
...
- **Support for SSE MCP servers**: You can now use SSE-based MCP servers, in addition to the previously supported command-based and HTTP-based MCP servers. Add and update MCP configurations from [Environments](https://roverlabs.com/environment)
...
- Fixes instances where Rover wasn't willing to use `gh` commands to interact with GitHub
...
- **Environment-Specific Setup Scripts**: You can now define a setup script from Environments without committing a `rover-setup.sh` file to your repository. Environment scripts take precedence over those defined in `rover-setup.sh` files.
...
- **Diff Context in GitHub mentions**: Mentioning @rover-labs in GitHub PR comments now includes context of the diff in the message back to Rover
...
- Rover now has permission to clone multiple repositories in a single sandbox
...
- **Updated Node version in the sandbox**: Node.js v22 is now the default version in the sandbox due to stability issues with v24. If your project requires Node v24, you can update your `rover-setup.sh` file to use it.
...
- **MCP server**: You can now use the rover cli as an MCP server. Run `claude mcp add rover -- rover mcp` to add it to Claude Code.
...
- Manual API key entry with `rover auth` for cases where automatic authentication fails.
...
- **Task Sharing**: Share your tasks with your collaborators:
  ...
  - You can change sharing permissions via the share modal on a task or in [Settings](https://roverlabs.com/settings)
- **Claude rate limit handling**: We now automatically queue and retry tasks if you hit your rate limit
- **New `rover list` CLI command**: View all tasks for the current repository
...
- **Anyone can signup**: Rover is free to use while in beta.
- **Revamped docs**: Much more comprehensive docs on how to use Rover
- **Improved GitHub integration**: Rover now automatically reacts with 👀 emoji when mentioned in GitHub comments
...
- **Progressive Web App support**: You can now add Rover to your home screen for faster access on mobile
...
- **Invite your team**: You can now generate access codes to invite your friends to try out Rover
...
- **New task creation command in cli**: Use `rover create` to create new tasks directly from the CLI with GitHub repository integration
- **Open in Claude**: `rover pull --resume` now opens a conversation in Claude locally after syncing the code
...
- Mention `@rover-labs` in GitHub comments to spawn new tasks
- Use the `rover` cli to continue working with the agent locally
