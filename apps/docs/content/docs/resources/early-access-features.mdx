...
If you want to preview some of the features the Rover team is actively working on, you can opt in to early access features:

### How to enable early access

<Steps>
  <Step>
    Navigate to [<Settings className="inline-block w-4 h-4 mx-1" /> **Settings**](https://www.roverlabs.com/settings)
  </Step>
  <Step>
    Find the `Early access features` setting and toggle the setting on
  </Step>
  <Step>
    Once enabled, an `Early Access` pill will appear at the top right of [<House className="inline-block w-4 h-4 mx-1" /> **Home**](https://www.roverlabs.com) and [<Settings className="inline-block w-4 h-4 mx-1" />**Settings**](https://www.roverlabs.com/settings)
  </Step>
</Steps>

## Features currently in early access

- **Branch Creation Toggle**: Control whether tasks create a new branch (the existing behavior) or work on the selected branch directly. Access this from below all prompt boxes. This is useful if you have an existing feature branch and you want Rover's git checkpointing system to commit changes to that branch instead of creating a new one.
