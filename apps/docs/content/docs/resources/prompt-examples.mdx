...
description: This collection demonstrates Rover's context capabilities through three essential examples that show how to leverage multiple context types effectively.
---
...
<Tabs items={['Example 1', 'Example 2', 'Example 3']}>
  <Tab value="Example 1">
    ...
    **Result**: Rover analyzes the mockup, understands the current implementation, and systematically updates all components to match the new design while preserving functionality.
  ...
  <Tab value="Example 2">
    ...
    **Why This Works**:
    ...
    - **Specific investigation areas** guide Rover efficiently
    - **Clear symptoms** help identify the problem
    - **Targeted file list** focuses the search

    **Result**: Rover fetches issue details and monitoring data, identifies the memory leak in event listener cleanup, and implements proper disposal patterns.
  ...
  <Tab value="Example 3">
    ...
    **Result**: Rover implements the entire notification system following existing patterns, matching the UI mockup, and fulfilling all requirements from the voice description.
...
<Callout type="info" title="Best Practices">
  ...
  2. **Provide Context**: Use file mentions to give Rover the full picture
...
- Start with smaller, focused prompts to understand Rover's capabilities
- Use file mentions (`@`) liberally - Rover works best with context
...
<Callout type="info" title="Remember">
  Rover excels when you provide rich context. The more relevant information
  you include, the better Rover can understand and complete your task.
</Callout>
