...
The Rover Daemon creates a unix socket at `/tmp/rover-daemon.sock` and listens for JSON messages containing configuration for Agent interactions. At a high-level, the daemon controls how to spawn a Agent process with the provided prompt and configuration and forwards the output to a specified API endpoint.
