...
export async function setupTestContainers(): Promise<SetupResult> {
  ...
  try {
    ...
    execSync(
      'docker exec rover_postgres_test psql -U postgres -d postgres -c "DROP SCHEMA IF EXISTS public CASCADE; CREATE SCHEMA public;"',
      {
        stdio: "inherit",
      },
    );
    // Clear Redis data
    execSync("docker exec rover_redis_test redis-cli FLUSHALL", {
      stdio: "inherit",
    });
  } catch (error) {
